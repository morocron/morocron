...
#Merge process
#-------------
echo -e "${yellow}Start merging release into ${MASTER_BRANCH_NAME}{norm}"
$GIT merge ${RELEASE_VERSION}-RELEASE -m "Merge release ${DEVELOP_BRANCH_NAME} into ${MASTER_BRANCH_NAME}"
if [ $? -eq 0 ]; then
    $GIT push
